...
const showstat = document.getElementById('stats1');
const statcont = document.getElementById('stat1');
const statholder = document.getElementById('statholder');
const openstats = document.getElementById('openstats');
const hidestats = document.getElementById('hidestats');
const bgbluredstat = document.getElementById('statcheck');
const closestat = stat1.querySelector('.toclose');

showstat.addEventListener('click', () => {
  if (screenMob > 428) {
    statholder.classList.toggle('openstat');
  
  if (statholder.classList.contains('openstat')) {
    openstats.style.display = 'none';
    hidestats.style.display = 'flex';
  } else {
    ...
    closestat.addEventListener('click', () => {
      statcont.style.opacity = '0';
      bgbluredstat.style.opacity = '0';
      setTimeout(function() {
        statcont.style.left = '-100%';
    }, 300);
    setTimeout(function() {
      bgbluredstat.style.right = '-100%';
      }, 300)
    })
  }
});
